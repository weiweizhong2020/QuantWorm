...
public class PlateView implements MouseListener, WindowListener {
	 ...
	 */
	public PlateView( ImagePlus imagePlus, List<WormInfo> wormsList, Dimension dimension, JFrame parentFrame, String resultsFolder, App application ) {
	...
	public ImagePlus updateImageAnnotations( ImagePlus imagePlus ) {
		...
		for( WormInfo worm : wormsList ) {
			...
			if( worm.label == 1 ) {
				...
				colorProcessor.setValue( Color.YELLOW.getRGB() );
			}; // if
	...
	public void popupMenuItemActionPerformed( ActionEvent actionEvent ) {
		...
		if( insideFlag == false ) {
			...
			// height of annotated image determines two regions
			int label = 0;
			if( y > ( nonAnnotatedImagePlus.getHeight() / 2 ) ) {
				label = 2;
			}
			else {
				label = 1;
			}; // if
			WormInfo worm = new WormInfo( howMany, x - size / 2, y - size / 2, size, size, size * size / 3, label );
			wormsList.add( worm );
			changesFlag = true;
		}; // if
